		...
		if(response) {
			...
			while((line = br.readLine()) != null) {
				...
				if(!t_ecoute.get(get).isEmpty()) {
					...
					int newLocalListening = Bytes.toInt(localListening) + Integer.parseInt(cols[2]);
					int newRadioListening = Bytes.toInt(radioListening) + Integer.parseInt(cols[3]);
					int newSkip = Bytes.toInt(skip) + Integer.parseInt(cols[4]);

					put.addColumn(CF_NAME, Bytes.toBytes("localListening"), Bytes.toBytes(newLocalListening));
					put.addColumn(CF_NAME, Bytes.toBytes("radioListening"), Bytes.toBytes(newRadioListening));
					put.addColumn(CF_NAME, Bytes.toBytes("skip"), Bytes.toBytes(newSkip));	
				} else {
					...
					put.addColumn(CF_NAME, Bytes.toBytes("localListening"), Bytes.toBytes(Integer.parseInt(cols[2])));
					put.addColumn(CF_NAME, Bytes.toBytes("radioListening"), Bytes.toBytes(Integer.parseInt(cols[3])));
					put.addColumn(CF_NAME, Bytes.toBytes("skip"), Bytes.toBytes(Integer.parseInt(cols[4])));
				}
				// ajout de la ligne
				t_ecoute.put(put);
				i += 1;    
